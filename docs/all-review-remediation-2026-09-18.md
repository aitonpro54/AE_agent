...
или save. Первая попытка вернуть его через `app.open()` зависла на CEP callback.
После отдельного явного разрешения пользователя AE был запущен без проекта,
исходный AEP открыт из работающего приложения, CEP перезагружен и read-back
подтвердил 1413 items, активный `CODX_133_MASTER` и `dirty=false`. Исходная
сессия восстановлена без записи клиентского файла.
...
4. Исходный клиентский AEP не мутировался и не сохранялся; после отдельного
   разрешения он снова открыт, CEP работает, read-back показывает clean state.
