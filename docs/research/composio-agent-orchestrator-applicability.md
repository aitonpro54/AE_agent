...
Milestone id/name: `M211 AO-inspired activity and session-state intake`
...
1. Add `.codex-audit/sdk-ao-pattern-intake/211-activity-session-schema.json`.
...
| --- | --- | --- | --- |
| M211 | Activity/session schema intake | общий язык состояний без runtime риска | AO install, worktree, PR, production edits |
| M212 | Stuck detector + escalation rules | снижает зависания Codex runs | auto-fix, auto-retry без лимитов |
| M213 | Status markdown/HTML dashboard | показывает sessions/blockers/next action | web server/dependency |
| M214 | Read-only CI/test failure analyzer | превращает logs в repair prompt | самостоятельное исправление кода analyzer-ом |
| M215 | Dummy worktree pilot outside production | проверяет isolation pattern | worktree в текущем repo |
| M216 | Optional AO sandbox pilot | проверяет настоящий AO feedback routing | подключение AO к AE Agent |
