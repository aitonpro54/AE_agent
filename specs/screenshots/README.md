...
- Provider tabs: `Gemini`, `OpenAI`, `Claude`, `OpenRouter`, `Local`. The visual rhythm still follows the four-tab reference, but OpenRouter is a product provider path and must stay visible.
- Gemini and Claude should be reachable API provider tabs in v1, with honest API-key setup states when keys are missing.
- `OpenAI` is active.
