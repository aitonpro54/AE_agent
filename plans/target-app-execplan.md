...
- Exact next prompt for M215: Continue in `C:\Users\Ant\Documents\Codex\AE_agent` with queue item `m215-ao-readonly-failure-analyzer`. Read `AGENTS.md`, `specs/target-app.md`, `plans/target-app-execplan.md`, `.codex/handoff.md`, `.codex-audit/sdk-roadmap-supervisor/211-ao-inspired-pipeline-hardening-queue.json`, `.codex-audit/sdk-ao-pattern-intake/212-activity-session-schema.json`, `.codex-audit/sdk-ao-pattern-intake/213-stuck-escalation-design.json`, and `.codex-audit/sdk-ao-pattern-intake/214-status-dashboard-contract.json`. Create `.codex-audit/sdk-ao-pattern-intake/215-readonly-failure-analyzer-contract.json`, add `scripts/create-roadmap-failure-analyzer-packet.js` and `scripts/sdk-ao-failure-analyzer-smoke.js`, update the queue closeout notes only if previous M212-M214 commits are observable in git log, update `plans/target-app-execplan.md` and `.codex/handoff.md`, and run the M215 validation commands. Do not auto-fix code, retry commands automatically, run live AE/CEP, install dependencies, create branches/worktrees, push, create a PR, GitHub issue, or GitHub Action, read broad unbounded logs by default, or touch CEP/live/production/dependency/package paths.
