...
- Пользователь просит add labeled items to render queue, queue labeled compositions, or add generated labeled comps to the render queue.
- Safe adaptation is acceptable: target compositions are explicit generated comp names or item indices proven with current typed project evidence before mutation.
- The requested label intent has already been translated into an explicit target comp list by the user or a prior safe planning step. Current typed tools do not expose project item label colors.
...
- This recipe is for generated compositions only; non-generated user assets, footage, folders, arbitrary label scans and ambiguous project inventory must fail closed.
- If the request needs true project-item label color discovery, project-panel selection reads, selected/labeled footage handling, render start, render queue deletion/reordering, arbitrary output templating, source relinking, save/saveAs, or exact source JSX behavior, fail closed and request a separate typed-tool contract.
...
4. Fail closed when targets are missing, duplicated, not compositions, not generated, derived only from uninspected label colors, selected only in the Project panel, or mixed with footage/folders/non-generated user assets.
5. Compute and disclose `renderQueueAddPlan`: target comp item indices/names, baseline render queue count, optional output path/template settings, expected new queue items, skipped targets and the label-discovery limitation.
...
9. Fail closed instead of using raw script execution when the request needs automatic label-color scanning, Project panel selection reads, adding footage or folders, rendering, queue cleanup, queue reordering, save/saveAs, or exact source JSX semantics.
...
- Treat label color selection as a typed-tool gap unless targets are already explicit. Do not infer project item labels from names or prior chat context.
- Do not use this recipe for render execution, render queue cleanup, project item label discovery, Project panel selection reads, selected footage/folder handling, source relinking, source replacement, template batch updates, user-asset cleanup, or raw script execution.
...
- Dry-run evidence lists accepted comp targets, skipped targets, baseline render queue count, optional output settings and the current label-discovery limitation.
...
- The plan reports label color discovery, Project panel selection reads, footage/folder queueing, non-generated user assets, render start, queue deletion/reordering, save/saveAs and exact source JSX semantics as typed-tool gaps.
