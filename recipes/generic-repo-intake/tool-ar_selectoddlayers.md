# tool-ar-selectoddlayers Intake Note

## Candidate

- Candidate id: `tool-ar-selectoddlayers`
- Queue alias: `tool-ar_selectoddlayers`
- Source path: `AR_SelectOddLayers.jsx`
- Typed plan: `recipes/ar-selectoddlayers-typed-plan.md`

## Safe Adaptation

The candidate name indicates selected active-composition layer selection for an
odd-indexed subset. In this detached child-run, the source JSX was not present
...
evidence, binds an explicit `oddSelectionPolicy`, computes concrete one-based
`oddLayerIndices`, applies selection only through `set_layer_selection`, and
...
ordering, zero-based or selected-ordinal odd semantics without explicit review,
