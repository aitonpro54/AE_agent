# tool-ar_distributekeyframestocomp Intake Note

## Candidate

- Candidate id: `tool-ar-distributekeyframestocomp`
- Queue alias: `tool-ar_distributekeyframestocomp`
- Source path: `AR_DistributeKeyframesToComp.jsx`
- Typed plan: `recipes/ar-distributekeyframestocomp-typed-plan.md`

## Safe Adaptation

The candidate name indicates selected-keyframe timing distribution across the
active composition timing bounds. In this detached child-run, the source JSX
was not present in the worktree, so the safe adaptation is limited to a
reviewed typed-plan pattern using existing active composition,
...
`distributeKeyframesToCompSpec`, computes reviewed
...
selection ordering, native comp UI side effects, unresolved same-time key
collisions, interpolation/ease or spatial tangent preservation without
