...
- The random subset has been converted into concrete one-based `randomLayerIndices` before mutation using a reviewed count, explicit reviewed sample, or deterministic seed plus documented sampling rule.
- The task does not require source-exact native UI randomness, hidden timeline state, persistent randomness, cross-comp selection, Project panel selection, layer edits, or raw JSX execution.
...
3. Define the eligible layer pool only from current typed layer inventory and reviewed user constraints.
4. Bind a reviewed `randomSelectionPolicy`, such as explicit `randomLayerIndices`, a finite `selectionCount`, or a deterministic seed plus documented sampling rule.
5. Compute `randomLayerIndices` before mutation, preserve eligible pool evidence, and fail closed when the pool is empty, truncated, ambiguous, or when the user requires unsupported nondeterministic behavior.
