...
npm.cmd run codex:orchestrator:ae-agent-feature-conveyor -- --item m190-full-ui-agent-new-tools-validation --validate-live --stage both --allow-mutating-live --approval-text "I approve one M190 Full UI Agent live conveyor validation run for new typed tools using OpenAI CLI and generated-only mutations"
npm.cmd run codex:orchestrator:ae-agent-feature-conveyor -- --item m191-mask-safety-live-validation --validate-live --stage both --allow-mutating-live --approval-text "I approve one M191 live CEP AE validation run for generated-only mask safety checks inside After Effects"
npm.cmd run codex:orchestrator:ae-agent-feature-conveyor -- --item m198-marker-lifecycle-live-validation --validate-live --stage both --allow-mutating-live --approval-text "I approve one M198 live CEP AE validation run for generated-only marker lifecycle checks using OpenAI CLI"
```
...
Feature and cleanup conveyor workspace-write child execution defaults to `gpt-5.3-codex` with `high` reasoning. The generated-only Full UI Agent live acceptance lanes stay on their explicit OpenAI CLI `gpt-5.5` commands.

M199-M211 add the AE Agent roadmap supervisor as the aggressive local autopilot layer above the existing bounded runners. The contract lives in `.codex-audit/sdk-roadmap-supervisor/199-roadmap-supervisor-contract.json` with schema `sdk-roadmap-supervisor-contract.v1`; readiness/governance/queue artifacts live in the same directory. The runner is deterministic control-plane code, not a long-lived LLM agent: one writer child runs at a time, up to two read-only reviewers may run in parallel, validation must pass before auto-commit, `.codex/handoff.md` must be updated when required, and push/dependency/live/CEP/external-provider work remains blocked without a separate per-item lane. The bounded writer/reviewer child profile uses `gpt-5.3-codex`: writers run `high` reasoning and read-only reviewers run `medium` reasoning. The default aggressive budget is `maxItems=3` and `maxMinutes=180`, with hard caps of 5 items and 300 minutes. M204 adds `roadmap-sdk`, a single approval chain runner that launches direct workspace-write children through `orchestrator/codex-sdk-orchestrator.mjs` with `workspace-write`, `approval=never`, `web-search=disabled`, `model=gpt-5.3-codex`, and `reasoning=high`. If the SDK stream/parser fails on local CLI transport noise, `--engine cli` now switches the `roadmap-sdk` writer itself to `codex exec` through `cmd.exe`, still with workspace-write, approval never, high reasoning, web search disabled, `gpt-5.3-codex`, and the same supervisor validation/commit gates. M209 records the duplicate_layers queue proof: plan-only preview creates no child run, SDK thread, or runtime state, and the roadmap supervisor smokes cover single-approval queue behavior, including wrong-approval rejection, live binding policy, unplanned path stops, ignored handoff handling, and Windows `.cmd` validation dispatch. M210 records duplicate_layers closeout state as docs/governance only: the queue current-state notes, README, and execution plan record committed M204-M209 evidence plus M210 local validation scope, while preserving no push, no PR, no dependency change, no runtime behavior change, and no CEP SDK write boundaries. M211 reconciles the tracked closeout state with the parent-updated `.codex/handoff.md`, so the duplicate_layers queue is closed rather than pending handoff repair.
...
npm.cmd run codex:orchestrator:ae-agent-roadmap-supervisor -- --live-check --json
npm.cmd run codex:orchestrator:ae-agent-roadmap-supervisor -- --execute-one --item <approved-item> --require-live-connectivity --approval-text "<exact supervisor approval>"
npm.cmd run codex:orchestrator:ae-agent-roadmap-supervisor -- --execute-one --engine cli --item <approved-item> --approval-text "<exact supervisor approval>"
npm.cmd run codex:orchestrator:ae-agent-roadmap-supervisor -- --run-until-budget --max-items 3 --max-minutes 180 --reviewers parallel --approval-text "I approve AE Agent roadmap supervisor repo=<absolute cwd> queue=<queue path> queueSha256=<sha256> itemIds=<ids> maxItems=3 maxMinutes=180 autoCommit=true noPush=true noDependencyChanges=true liveBindings=<bindings|none> noLiveCepAeUnlessPerItemApproved=true"
npm.cmd run codex:orchestrator:ae-agent-roadmap-supervisor -- --mission-plan-only --mission-prompt "<goal>" --max-items 1 --json
npm.cmd run codex:orchestrator:ae-agent-roadmap-supervisor -- --mission-run --queue <queue path> --max-items 3 --max-minutes 180 --approval-text "I approve AE Agent roadmap supervisor mission repo=<absolute cwd> missionSha256=<sha256|none> queue=<queue path> queueSha256=<sha256> maxItems=3 maxMinutes=180 autoCommit=true autoFollowups=bounded livePolicy=generatedOnlyOpenAiCli liveBindings=<bindings|none> noPush=true noDependencyChanges=true noUserAssetMutation=true"
```

`--plan-only` prints the exact approval text for the selected repository, queue, queueSha256, queue item ids, item/time budget, auto-commit permission, generated-only liveBindings, and hard bans: `noPush=true`, `noDependencyChanges=true`, and `noLiveCepAeUnlessPerItemApproved=true`. Any queue edit changes `queueSha256` and invalidates the old approval. `--live-check` runs read-only validation against the already-open After Effects / AE Agent panel using `node scripts/cep-panel-cdp-smoke.js inspect` and `node scripts/cep-panel-cdp-smoke.js connector-status-smoke`, then writes the report under `.codex-runtime/sdk/roadmap-supervisor/<session-id>/`. `--execute-one` runs exactly one approved queue item and stops after commit/report. `--run-until-budget` resumes from `.codex-runtime/sdk/roadmap-supervisor/<session-id>/state.json`, commits after every completed item, and stops on the first dirty tree, approval mismatch, out-of-scope path, failed validation, missing handoff, reviewer blocking finding, or budget gate. Real generated-only AE/CEP validation can be attached to a queue item through `liveValidation`; mutating/OpenAI CLI lanes either appear in `liveBindings` with `generatedOnlyLive=true` and `noUserAssetMutation=true` or require their own exact `--live-approval-text` fallback.

`--mission-plan-only` and `--mission-run` provide the universal mission layer for "do it through the supervisor" prompts. A mission approval binds the repository, mission prompt hash or existing queue, queue hash, item/time budget, bounded follow-up permission, generated-only OpenAI CLI live policy, live bindings, no push, no dependency changes, and no user asset mutation. Prompt missions synthesize and separately commit a tracked queue artifact under `.codex-audit/sdk-roadmap-supervisor/` before execution. Existing-queue missions skip the manual plan-only copy/paste loop and execute directly from the mission approval. Mission runtime state records phases such as intake, queue creation, plan verification, item/reviewer/validation/live work, bounded follow-up, handoff, commit, and stop. Generated-only live lanes run their read-only CEP connectivity checks before item live commands; unsafe live policies still fail closed during queue validation.
