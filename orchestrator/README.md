...
`sdk-write` uses the same operation envelope and is accepted only for `scope:"docs-audit"` or `scope:"orchestrator"` with `mode:"sdk-write"`. M117R generalizes docs-audit planned outputs from the original M115 single-file allowlist to any non-empty `plannedPaths` set under `.codex-audit/**`, after the same unsafe path shape checks, forbidden path checks, and docs-audit scope allowlist validation. M123 adds the first orchestrator-scope SDK write lane, but only for docs-only Markdown planned paths under `orchestrator/**`, such as `orchestrator/m123-sdk-thread-orchestrator-scope-output.md`; orchestrator source files like `.mjs`/`.js` and paths such as `src/**`, `scripts/**`, `specs/**`, `cep-panel/**`, `../outside.md`, `.env`, `node_modules/**`, and `.git/**` are rejected before SDK thread creation. The SDK prompt is constrained to the validated planned path set, auto-commit remains disabled, and the runner captures pre/post git snapshots plus `git diff --check`. After the SDK turn, the runner hard-stops if the changed-since-pre diff includes anything outside the planned path set.
